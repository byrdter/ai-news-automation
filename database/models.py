...
def create_default_news_sources(session: Session):
    """Create default news sources for the AI News Automation System"""
    ...
    default_sources = [
        ...
        NewsSource(
            ...
            rss_feed_url="https://deepmind.com/blog/feed/basic/",
            tier=1,
            category="AI Research"
        ),
        NewsSource(
            ...
            rss_feed_url="https://www.anthropic.com/news/rss.xml",
            tier=1,
            category="AI Research"
        ),
        NewsSource(
    ...
    # Add sources to database if they don't already exist
    for source in default_sources:
        existing = session.query(NewsSource).filter_by(name=source.name).first()
        if not existing:
            session.add(source)
